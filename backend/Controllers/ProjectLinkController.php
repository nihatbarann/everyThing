<?php

require_once __DIR__ . '/../Core/Database.php';
require_once __DIR__ . '/../Core/AuthMiddleware.php';
require_once __DIR__ . '/ActivityLogController.php';

class ProjectLinkController {

    private $auth;

    public function __construct() {
        $this->auth = new AuthMiddleware();
    }

    private function canAccess(PDO $pdo, $projectId, $user) {
        if ($this->auth->isAdmin($user)) return true;

        $stmt = $pdo->prepare("SELECT id FROM projects WHERE id = ? AND created_by = ?");
        $stmt->execute([$projectId, $user['user_id']]);
        if ($stmt->fetch()) return true;

        $stmt = $pdo->prepare("SELECT 1 FROM project_members WHERE project_id = ? AND user_id = ?");
        $stmt->execute([$projectId, $user['user_id']]);
        return (bool) $stmt->fetch();
    }

    /**
     * GET /api/projects/{projectId}/links
     */
    public function index($projectId) {
        $user = $this->auth->verifyToken();

        try {
            $pdo = Database::getConnection();
            if (!$this->canAccess($pdo, $projectId, $user)) {
                http_response_code(403);
                echo json_encode(['error' => 'Yetkiniz yok']);
                return;
            }

            $stmt = $pdo->prepare("SELECT * FROM project_links WHERE project_id = ? ORDER BY is_favorite DESC, title ASC");
            $stmt->execute([$projectId]);
            echo json_encode(['links' => $stmt->fetchAll(PDO::FETCH_ASSOC)]);
        } catch (Exception $e) {
            http_response_code(500);
            echo json_encode(['error' => 'Server error']);
        }
    }

    /**
     * POST /api/projects/{projectId}/links
     */
    public function create($projectId) {
        $user = $this->auth->verifyToken();
        $data = json_decode(file_get_contents('php://input'), true);

        try {
            $pdo = Database::getConnection();
            if (!$this->canAccess($pdo, $projectId, $user)) {
                http_response_code(403);
                echo json_encode(['error' => 'Yetkiniz yok']);
                return;
            }

            $title = trim($data['title'] ?? 'Yeni Link');
            $category = trim($data['category'] ?? '') ?: null;
            $isFavorite = !empty($data['is_favorite']) ? 1 : 0;

            $stmt = $pdo->prepare(
                "INSERT INTO project_links (project_id, title, category, url, username, password, notes, is_favorite, created_by)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
            );
            $stmt->execute([
                $projectId, $title, $category,
                trim($data['url'] ?? ''),
                $data['username'] ?? '',
                $data['password'] ?? '',
                $data['notes'] ?? '',
                $isFavorite,
                $user['user_id']
            ]);
            $newId = $pdo->lastInsertId();

            ActivityLogController::log($user['user_id'], $user['username'], 'project_link.create', "Created project link \"{$title}\"", 'project_link', $newId);

            echo json_encode(['success' => true, 'id' => $newId]);
        } catch (Exception $e) {
            http_response_code(500);
            echo json_encode(['error' => 'Server error: ' . $e->getMessage()]);
        }
    }

    /**
     * PUT /api/projects/{projectId}/links/{id}
     */
    public function update($projectId, $id) {
        $user = $this->auth->verifyToken();
        $data = json_decode(file_get_contents('php://input'), true);

        try {
            $pdo = Database::getConnection();
            if (!$this->canAccess($pdo, $projectId, $user)) {
                http_response_code(403);
                echo json_encode(['error' => 'Yetkiniz yok']);
                return;
            }

            $stmt = $pdo->prepare(
                "UPDATE project_links SET title=?, category=?, url=?, username=?, password=?, notes=?, is_favorite=? WHERE id=? AND project_id=?"
            );
            $stmt->execute([
                trim($data['title'] ?? ''),
                trim($data['category'] ?? '') ?: null,
                trim($data['url'] ?? ''),
                $data['username'] ?? '',
                $data['password'] ?? '',
                $data['notes'] ?? '',
                !empty($data['is_favorite']) ? 1 : 0,
                $id, $projectId
            ]);

            echo json_encode(['success' => true]);
        } catch (Exception $e) {
            http_response_code(500);
            echo json_encode(['error' => 'Server error']);
        }
    }

    /**
     * DELETE /api/projects/{projectId}/links/{id}
     */
    public function delete($projectId, $id) {
        $user = $this->auth->verifyToken();

        try {
            $pdo = Database::getConnection();
            if (!$this->canAccess($pdo, $projectId, $user)) {
                http_response_code(403);
                echo json_encode(['error' => 'Yetkiniz yok']);
                return;
            }

            $pdo->prepare("DELETE FROM project_links WHERE id = ? AND project_id = ?")->execute([$id, $projectId]);

            ActivityLogController::log($user['user_id'], $user['username'], 'project_link.delete', "Deleted project link ID {$id}", 'project_link', $id);

            echo json_encode(['success' => true]);
        } catch (Exception $e) {
            http_response_code(500);
            echo json_encode(['error' => 'Server error']);
        }
    }
}
